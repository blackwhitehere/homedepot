...
global log_handler
## libfm
libfm_exe = config.root + "libfm-1.40.windows/libfm.exe"

## rgf
call_exe = config.root + "rgf1.2/test/call_exe.pl"
rgf_exe = config.root + "rgf1.2/bin/rgf.exe"
        ...
        for fold in range(1, config.n_folds + 1):
            ...
            for n in range(bagging_size):
                ...
                if param["task"] in ["regression", "ranking"]:
                    ...
                    pred = bst.predict(dvalid_base)
                    w = np.asarray(range(1, config.n_classes + 1))
