...
def seg_words(str1, str2):  #returns a string that contains word "parts" that appear both in str1 and str2
    str2 = str2.lower()
    ...
    for word in words:
        if len(word) > 3:
            ...
            if len(s) > 1:  # s stores the words from str1 that appear as parts of the words from str2
                s += [z for z in s1 if z not in ['er', 'ing', 's', 'less'] and len(z) > 1]
            else:
                s.append(word)
        else:
            s.append(word)
    return (" ".join(s))  # returns string from a sequence seperated by space


def segmentit(s, txt_arr, t):  # if word s appears as part of any word in txt_arr add the common part to the return list
    st = s
    r = []
...
# used to count total co-occurrence of words in the query in title and description, not frequency!

def str_common_word(str1, str2):
    words, cnt = str1.split(), 0 #split words in str1 and set count to 0
    for word in words: # check all words in str1
        if str2.find(word) >= 0: # non negative if word is found
            cnt += 1
    return cnt #total count of passed tests

# used to count frequency of the last query term in title and description

def str_whole_word(str1, str2, i_): # counts how many times str1 is in str2 from ith position of str2
    cnt = 0
    while i_ < len(str2): #start finding from position i_
        i_ = str2.find(str1, i_)
        if i_ == -1: #if not found
            return cnt
        else:
            cnt += 1
            i_ += len(str1)
...
df_all['product_info'] = df_all['search_term'] + "\t" + df_all['product_title'] + "\t" + df_all[
    'product_description']  # + "\t" df_all['flat_attr']
print("--- Prod Info as join of query, title and description: %s minutes ---" % round(((time.time() - start_time) / 60), 2))

df_all['len_of_query'] = df_all['search_term'].map(lambda x: len(x.split())).astype(np.int64)
df_all['len_of_title'] = df_all['product_title'].map(lambda x: len(x.split())).astype(np.int64)
df_all['len_of_description'] = df_all['product_description'].map(lambda x: len(x.split())).astype(np.int64)
df_all['len_of_brand'] = df_all['brand'].map(lambda x: len(x.split())).astype(np.int64)
print("--- Len of str fields: %s minutes ---" % round(((time.time() - start_time) / 60), 2))

# search_term holds now the common word segments with product title
df_all['search_term'] = df_all['product_info'].map(lambda x: seg_words(x.split('\t')[0], x.split('\t')[1]))
print("--- Transform Query into word segments shared with title: %s minutes ---" % round(((time.time() - start_time)/60), 2))

# frequency of the full query in title and description
df_all['query_in_title'] = df_all['product_info'].map(lambda x: str_whole_word(x.split('\t')[0], x.split('\t')[1], 0))
df_all['query_in_description'] = df_all['product_info'].map(
    lambda x: str_whole_word(x.split('\t')[0], x.split('\t')[2], 0))
print("--- Full query appearance in title and description: %s minutes ---" % round(((time.time() - start_time) / 60), 2))

# 0/1 if the last query term is found in title and description, not frequency!
...
# combined count of times query terms are (simply) found in title and description, not frequency!
df_all['word_in_title'] = df_all['product_info'].map(lambda x: str_common_word(x.split('\t')[0], x.split('\t')[1]))
df_all['word_in_description'] = df_all['product_info'].map(
    lambda x: str_common_word(x.split('\t')[0], x.split('\t')[2]))

# percentage overlap between query title
df_all['ratio_title'] = df_all['word_in_title'] / df_all['len_of_query']
# percentage overlap between query and description
df_all['ratio_description'] = df_all['word_in_description'] / df_all['len_of_query']

# add TFIDF features for title, description and attributes TODO
#
#
#
...
# count of times query terms segments (that are found in product title) are found in brand, not frequency
df_all['word_in_brand'] = df_all['attr'].map(lambda x: str_common_word(x.split('\t')[0], x.split('\t')[1]))
# determine percentage overlap between query and brand
        ...
        if b_ not in stop_ and len(b_) > 0:
            if b_ not in d:
                d[b_] = [1, str_common_word(b_, df_outliers['product_title'][i]),
                         str_common_word(b_, df_outliers['brand'][i]),
                         str_common_word(b_, df_outliers['product_description'][i])]
            else:
                d[b_][0] += 1
                d[b_][1] += str_common_word(b_, df_outliers['product_title'][i])
...
for i in range(len(ds2)):
    f.write(ds2.index[i] + "|" + str(ds2["count"][i]) + "|" + str(ds2["in title"][i]) + "|" + str(
        ds2["in brand"][i]) + "|" + str(ds2["in prod"][i]) + "\n")
f.close()
